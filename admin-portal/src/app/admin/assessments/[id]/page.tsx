...
export default function AssessmentDashboardPage() {
  ...
  const downloadExcelReport = () => {
    ...
    if (excludeZeroScores) params.append("excludeZero", "true");
    const queryString = params.toString();
    window.open(`${baseUrl}/api/v1/candidates/export-comprehensive/${assessmentId}${queryString ? `?${queryString}` : ""}`, "_blank");
  };
  ...
  const filteredCandidates = (candidates || []).filter((c: any) => {
    ...
    if (statusFilter === "LOCKED" && c.status !== "LOCKED") return false;
    if (statusFilter === "INVITED" && !(c.emailStatus === "SENT" || c.emailStatus === "DELIVERED")) return false;
    if (statusFilter === "NOT_STARTED" && c.attempt) return false;

    // Exclude 0% scores
    if (excludeZeroScores) {
      const att = c.attempt || c.attempts?.[0];
      if (!att) return false;
    ...
    <div style={{ padding: "28px 36px", background: "#F8FAFC", minHeight: "calc(100vh - 64px)" }}>
      ...
      <div style={{ background: "white", padding: "24px 28px", borderRadius: "16px", border: "1px solid #E2E8F0", marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "#64748B", margin: 0 }}>{assessment.description || "Talent Assessment Session"}</p>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", marginBottom: "18px", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flex: 1, minWidth: "260px" }}>
          ...
          {/* Exclude 0% Scores Toggle */}
          <label style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "8px 12px", borderRadius: "10px", border: excludeZeroScores ? "1px solid #FCD34D" : "1px solid #CBD5E1", background: excludeZeroScores ? "#FFFBEB" : "white", fontSize: "12px", fontWeight: 700, color: excludeZeroScores ? "#92400E" : "#475569", cursor: "pointer", userSelect: "none" }}>
            <input
              type="checkbox"
              checked={excludeZeroScores}
              onChange={(e) => setExcludeZeroScores(e.target.checked)}
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button
            ...
            title={`Download 5-Sheet Report (${filteredCandidates.length} filtered candidate${filteredCandidates.length === 1 ? '' : 's'})`}
          ...
          >
            <Download size={14} /> Download 5-Sheet Report ({filteredCandidates.length})
          </button>
