  ...
  const initializeExamSession = async (identifier: string) => {
    try {
      ...
      if (data.success) {
        ...
        if (data.tenant) {
          setCandidate((prev: any) => ({ ...prev, tenant: data.tenant }));
        }
  ...
  return (
    <div className="test-page">
      <Navbar
        ...
        logoUrl={candidate?.tenant?.logoUrl}
        tenantName={candidate?.tenant?.name}
        portalTitle={candidate?.tenant?.portalTitle}
        primaryColor={candidate?.tenant?.primaryColor}
      />
