...
export class CandidatesService {
  ...
  // ─── GET CANDIDATES ────────────────────────────────────────────────────────
  async getCandidates(assessmentId?: string, vendorId?: string, tenantId?: string) {
    const whereClause: any = { isDeleted: false };
    if (tenantId) whereClause.assessment = { tenantId };
  ...
  async verifyAndStartExam(data: {
    ...
    const tenantInfo = candidate.assessment?.tenant || sessionData?.tenant || null;
  ...
  async startExamSession(candidateIdentifier: string) {
    ...
    const tenantInfo = candidate.assessment?.tenant || null;
  ...
  async getAllAssessments() {
    ...
    return assessments.map((ass) => {
      ...
      return {
        ...
        passingPercentage: ass.passingPercentage ?? 50.0,
        maxProctorWarnings: ass.maxProctorWarnings ?? 6,
        enableCamera: (ass as any).enableCamera ?? true,
        enableFullscreen: (ass as any).enableFullscreen ?? true,
        enableTabSwitch: (ass as any).enableTabSwitch ?? true,
        enableCopyPaste: (ass as any).enableCopyPaste ?? true,
        questionBankId: ass.questionBankId || null,
  ...
  async getAssessmentByIdentifier(identifier: string) {
    ...
    return {
      ...
      totalQuestions: TOTAL_QUESTIONS,
      passingPercentage: assessment.passingPercentage ?? 50.0,
      maxProctorWarnings: assessment.maxProctorWarnings ?? 6,
      enableCamera: (assessment as any).enableCamera ?? true,
      enableFullscreen: (assessment as any).enableFullscreen ?? true,
      enableTabSwitch: (assessment as any).enableTabSwitch ?? true,
      enableCopyPaste: (assessment as any).enableCopyPaste ?? true,
      questionBankId: assessment.questionBankId || null,
      isExpired,
      isNotStarted,
      uniqueCandidateLink: `${frontendBaseUrl}/${assessment.slug || assessment.id}`,
      tenant: assessment.tenant || null,
    };
  }
  ...
  async verifyCandidateToken(token: string, inputEmail?: string) {
    ...
    const assessment: any = candidate.assessment;
    const now = new Date();
  ...
  async exportComprehensiveExcel(options?: {
    assessmentId?: string;
    status?: string;
    excludeZero?: boolean;
    vendorId?: string;
  } | string): Promise<ExcelJS.Buffer> {
    const opts = typeof options === 'string' ? { assessmentId: options } : (options || {});
    const { assessmentId, status, excludeZero, vendorId } = opts;
    ...
    const whereClause: any = { isDeleted: false };
    if (assessmentId && assessmentId !== 'all' && assessmentId !== 'ALL') {
    ...
    const candidates = allCandidates.filter((c) => {
      ...
      const effStatus = att?.status || c.status;

      // Status filter
      if (status && status !== 'ALL' && status !== 'all') {
        const normStatus = status.toUpperCase();
        if (normStatus === 'COMPLETED') {
          if (c.status !== 'COMPLETED' && att?.status !== 'COMPLETED') return false;
        } else if (normStatus === 'LOCKED') {
          if (c.status !== 'LOCKED' && att?.status !== 'LOCKED') return false;
        } else if (normStatus === 'IN_PROGRESS') {
          if (c.status !== 'IN_PROGRESS' && att?.status !== 'IN_PROGRESS') return false;
        } else if (normStatus === 'REGISTERED' || normStatus === 'NOT_STARTED') {
          if (c.status !== 'REGISTERED' && att) return false;
        } else if (normStatus === 'DISQUALIFIED') {
          if (c.status !== 'DISQUALIFIED' && att?.status !== 'DISQUALIFIED') return false;
        } else if (effStatus !== normStatus && c.status !== normStatus) {
          return false;
        }
      }
      ...
      if (excludeZero) {
        ...
        if (score <= 0 && pct <= 0) return false;
      }

      return true;
    });

    const targetAssessment = (assessmentId && assessmentId !== 'all' && assessmentId !== 'ALL')
    ...
    sheet1.columns = [{ width: 28 }, { width: 35 }, { width: 18 }, { width: 18 }];

    const orgReportName = (targetAssessment?.tenant?.name || 'TALENT ASSESSMENT').toUpperCase();
    sheet1.addRow([`${orgReportName} — ASSESSMENT SUMMARY REPORT`]);
    ...
    sheet1.addRow(['Status Filter Applied', status && status !== 'ALL' ? status.toUpperCase() : 'All Statuses']);
    sheet1.addRow(['Score Filter Applied', excludeZero ? 'Yes (Excluded 0% / Zero Scores)' : 'None (All Scores Included)']);
    ...
    sheet1.addRow(['Total Exported Candidates', totalCands, '100%']);
