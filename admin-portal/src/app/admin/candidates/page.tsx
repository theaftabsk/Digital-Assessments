...
export default function CandidatesManagementPage() {
  ...
  const downloadExcelReport = () => {
    ...
    const effectiveVendor = userRole === "VENDOR" ? loggedVendorId : (selectedVendorId !== "ALL" ? selectedVendorId : null);
    if (effectiveVendor) params.append("vendorId", effectiveVendor);

    const queryString = params.toString();
    const url = `${baseUrl}/api/v1/candidates/export-comprehensive/${target}${queryString ? `?${queryString}` : ""}`;
  ...
  const filteredCandidates = candidates.filter((c) => {
    ...
    // Status filter
    if (statusFilter === "LOCKED" && !(c.status === "LOCKED" || c.attempt?.status === "LOCKED")) return false;
    if (statusFilter === "COMPLETED" && c.status !== "COMPLETED") return false;
    if (statusFilter === "IN_PROGRESS" && c.status !== "IN_PROGRESS") return false;
    if (statusFilter === "REGISTERED" && !(c.status === "REGISTERED" || !c.attempt)) return false;
    if (statusFilter === "DISQUALIFIED" && c.status !== "DISQUALIFIED") return false;
    if (statusFilter !== "ALL" && statusFilter !== "LOCKED" && statusFilter !== "COMPLETED" && statusFilter !== "IN_PROGRESS" && statusFilter !== "REGISTERED" && statusFilter !== "DISQUALIFIED" && c.status !== statusFilter) {
      return false;
    }

    // Exclude 0% overall scores
