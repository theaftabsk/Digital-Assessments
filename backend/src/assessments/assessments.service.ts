...
export class AssessmentsService {
  ...
  async getAssessments(vendorIdentifier?: string, tenantId?: string) {
    const whereClause: any = {
      status: { not: 'ARCHIVED' },
    };

    // Strictly scope by tenantId if provided
    if (tenantId) {
  ...
  async getAssessmentById(id: string, tenantId?: string) {
    const whereClause: any = {
      OR: [{ id }, { slug: id }],
    };
  ...
  async saveAssessment(
    ...
    if (data.id && data.status === 'ACTIVE') {
      ...
      if (existing) {
        if (userTenantId && existing.tenantId !== userTenantId) {
          throw new ForbiddenException('Cannot modify assessment belonging to another organization.');
        }
    ...
    if (data.id) {
      ...
      if (!resolvedTenantId) {
        const defaultTenant = await this.getOrCreateTenant();
    ...
    return this.getAssessmentById(assessment.id, userTenantId);
  }

  async deleteAssessment(id: string, userRole?: string, userTenantId?: string) {
    if (userRole === 'VENDOR') {
    ...
    if (userTenantId && existing.tenantId !== userTenantId) {
      throw new ForbiddenException('Cannot delete assessment belonging to another organization.');
    }
