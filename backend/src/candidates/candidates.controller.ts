import { Controller, Get, Post, Delete, Body, Query, Param, Res, Headers, Req } from '@nestjs/common';
...
export class CandidatesController {
  ...
  async exportAllComprehensiveExcel(
    @Query('status') status: string | undefined,
    @Query('excludeZero') excludeZero: string | undefined,
    @Query('vendorId') vendorId: string | undefined,
    @Headers('x-vendor-id') headerVendorId: string | undefined,
    @Res() res: Response,
  ) {
    const effectiveVendorId = vendorId || headerVendorId;
    const buffer = await this.candidatesService.exportComprehensiveExcel({
      status,
      excludeZero: excludeZero === 'true' || excludeZero === '1',
  ...
  async getCandidates(
    ...
    const effectiveVendorId = vendorId || headerVendorId || auth.vendorId;
    const candidates = await this.candidatesService.getCandidates(assessmentId, effectiveVendorId, auth.tenantId);
